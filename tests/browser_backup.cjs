...
(async () => {
 ...
 try {
  ...
  const inclusion=settings.getByRole('checkbox',{name:'Danbooru backup inclusion',exact:true});
  const before=await page.evaluate(async()=>(await(await fetch('/api/backups')).json()).components);
  await inclusion.click();
  await page.waitForFunction(()=>!document.querySelector('[aria-label="Danbooru backup inclusion"]').disabled);
  assert.equal(await inclusion.getAttribute('aria-checked'),'false');
  assert.equal(await moduleDialog('Danbooru').count(),0);
  await page.reload(); await open();
  await inclusion.focus(); await page.keyboard.press('Space');
  await page.waitForFunction(()=>!document.querySelector('[aria-label="Danbooru backup inclusion"]').disabled);
  const after=await page.evaluate(async()=>(await(await fetch('/api/backups')).json()).components);
  assert.deepEqual(after,before);
  assert.equal(await inclusion.getAttribute('aria-checked'),'mixed');
  check('module checkbox toggles without opening a dialog and restores partial selection after reload by keyboard');
  ...
  assert.equal(await moduleDialog('Files').getByRole('button',{name:'Select all',exact:true}).count(),0);
  assert.equal(await moduleDialog('Files').getByRole('button',{name:'Select none',exact:true}).count(),0);
  await moduleDialog('Files').getByRole('checkbox',{name:'File attachments',exact:true}).click();
  await page.locator('.backup-overlay').click({position:{x:5,y:5}});
  await moduleDialog('Files').waitFor({state:'detached'});
  assert.equal(await settings.getByRole('checkbox',{name:'Files backup inclusion'}).getAttribute('aria-checked'),'true');
  await settings.getByRole('button',{name:'Files',exact:true}).click();
  ...
  assert.equal(await settings.getByRole('button',{name:'Checkpoint now',exact:true}).count(),0);
  check('real scratch restore preserves browser preferences and recovery results without standalone checkpoint controls');
