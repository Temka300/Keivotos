...
(async()=>{
 ...
 try{
  ...
  recoveryMissing=true;await section('Backup').click();await page.getByText(/Recovery checkpoints is unavailable/).waitFor();
  assert.equal(await page.getByRole('button',{name:'Back up now',exact:true}).isEnabled(),true);
  check('failed recovery-status request leaves backup controls usable');
