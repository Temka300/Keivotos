              ...
              <div class="flex flex-wrap items-center justify-between gap-4">
                <div class="flex flex-wrap items-center gap-3">
                  ...
                  <div class="flex flex-wrap gap-2">
                    <span class="rounded-full border border-cyan-300/10 bg-cyan-500/[0.07] px-2.5 py-1 text-[11px] text-cyan-100">{foldersLoaded ? `${libraryFolders.length} ${$settingsStyle === 'modern' ? 'folder' : 'root'}${libraryFolders.length === 1 ? '' : 's'}` : 'Loading folders…'}</span>
...
{:else if selectedSection === 'maintenance'}
          <div class="mx-auto max-w-3xl space-y-4">
            <section class="overflow-hidden rounded-xl border border-[#292938] bg-[#111118]">
              <div class="border-b border-[#242432] px-4 py-3"><h4 class="text-sm font-semibold text-gray-200">{$settingsStyle === 'modern' ? 'Library tools' : 'Recovery maintenance'}</h4></div>
              {#if refreshTool}
